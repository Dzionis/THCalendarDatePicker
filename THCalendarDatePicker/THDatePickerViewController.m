...
+ (THDatePickerViewController *)datePicker {
  return [[THDatePickerViewController alloc]
      initWithNibName:@"THDatePickerViewController"
...
- (void)setAutoCloseOnSelectDate:(BOOL)autoClose {
  if (!_allowClearDate) [self setAllowClearDate:!autoClose];
  _autoCloseOnSelectDate = autoClose;
  if (_autoCloseOnSelectDate) _allowMultiDaySelection = NO;
}
...
- (void)setDateRangeFrom:(NSDate *)fromDate toDate:(NSDate *)toDate {
  ...
  NSDate *intFromDate =
      [(fromDate ? fromDate : self.internalDate)dateWithOutTime];
  NSDate *intToDate = [(toDate ? toDate : self.internalDate)dateWithOutTime];
  [self setDaysInHistorySelection:[[self.internalDate dateWithOutTime]
                                      daysFromDate:intFromDate]];
  [self setDaysInFutureSelection:[intToDate
                                     daysFromDate:[self.internalDate
                                                          dateWithOutTime]]];
...
- (void)viewDidLoad {
  ...
  [self.okBtn
      setImage:(SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"8.0")
                    ? [UIImage imageNamed:(_autoCloseOnSelectDate
                                               ? @"dialog_clear"
                                               : @"dialog_ok")
                                               inBundle:[NSBundle
                                                            bundleForClass:
                    ...
                    : [UIImage
                          imageNamed:(_autoCloseOnSelectDate ? @"dialog_clear"
                                                             : @"dialog_ok")])
      forState:UIControlStateNormal];

  self.okBtn.hidden = _controlButtonsHidden;
  self.closeBtn.hidden = _controlButtonsHidden;
}
...
- (void)configureButtonAppearances {
  [super viewDidLoad];
  [[self.nextMonthBtn imageView]
      setContentMode:UIViewContentModeScaleAspectFit];
  [[self.prevMonthBtn imageView]
      setContentMode:UIViewContentModeScaleAspectFit];
...
- (void)setClearControlButtons:(BOOL)dontShowButtons {
  _controlButtonsHidden = dontShowButtons;
}
...
- (void)redrawDays {
  ...
  NSDate *date = [_calendar dateByAddingComponents:offsetComponents
  ...
  for (int i = 0; i < days; i++) {
    ...
    THDateDay *day = [[[NSBundle bundleForClass:self.class]
        loadNibNamed:@"THDateDay"
               owner:self
    ...
    if (self.currentDateColor) [day setCurrentDateColor:self.currentDateColor];
    ...
    if (!_allowMultiDaySelection) {
      if (_internalDate &&
          ![[self dateWithOutTime:date] timeIntervalSinceDate:_internalDate]) {
        self.currentDay = day;
        [day setSelected:YES];
      }
    } else {
    ...
    [day indicateDayHasItems:(_dateHasItemsCallback &&
                              _dateHasItemsCallback(date))];

    NSDateComponents *comps =
        [_calendar components:NSCalendarUnitDay fromDate:date];
    [day.dateButton
        setTitle:[NSString stringWithFormat:@"%ld", (long)[comps day]]
        forState:UIControlStateNormal];
...
- (void)redrawWeekdays:(int)dayWidth {
  if (!self.weekdaysView.subviews.count) {
    ...
    NSDateComponents *comps =
        [_calendar components:NSCalendarUnitDay fromDate:[NSDate date]];
    NSCalendar *c =
...
- (NSDate *)date {
  if (!self.internalDate)
    ...
    NSDateComponents *internalComps =
        [_calendar components:ymd fromDate:self.internalDate];
    int time = NSCalendarUnitHour | NSCalendarUnitMinute |
               NSCalendarUnitSecond | NSCalendarUnitTimeZone;
    NSDateComponents *origComps = [_calendar
        components:time
          fromDate:(_allowMultiDaySelection && [_selectedDates count] > 0
                        ? [_selectedDates lastObject]
                        : _date)];
...
- (BOOL)shouldOkBeEnabled {
  if (!_allowMultiDaySelection) {
    if (_autoCloseOnSelectDate) return YES;
    NSLog(@"interval %f",
          [self.internalDate timeIntervalSinceDate:_dateNoTime]);
    return (self.internalDate && _dateNoTime &&
            (_allowSelectionOfSelectedDate ||
             [self.internalDate timeIntervalSinceDate:_dateNoTime])) ||
...
- (void)setDisplayedMonth:(int)month year:(int)year {
  ...
  [df setCalendar:[NSCalendar
                      calendarWithIdentifier:NSCalendarIdentifierGregorian]];
  self.firstOfCurrentMonth =
      [df dateFromString:[NSString stringWithFormat:@"%d-%@%d", year,
                                                    (month < 10 ? @"0" : @""),
                                                    month]];
...
- (void)storeDateInformation {
  NSDateComponents *comps =
      [_calendar components:NSCalendarUnitWeekday | NSCalendarUnitDay
                   fromDate:self.firstOfCurrentMonth];
  NSCalendar *c =
      [NSCalendar calendarWithIdentifier:NSCalendarIdentifierGregorian];
#ifdef DEBUG
//[c setFirstWeekday:FIRST_WEEKDAY];
  ...
  if (bufferDaysBeginning < 0) bufferDaysBeginning += 7;
...
- (void)incrementMonth:(int)incrValue {
  ...
  NSDate *incrementedMonth =
      [_calendar dateByAddingComponents:offsetComponents
                                 toDate:self.firstOfCurrentMonth
                                options:0];
...
- (void)dateDayTapped:(THDateDay *)dateDay {
  ...
  if (!_allowMultiDaySelection) {
    if (!_internalDate || [_internalDate timeIntervalSinceDate:dateDay.date] ||
        _allowSelectionOfSelectedDate) {  // new date selected
      [self.currentDay setSelected:NO];
      [self.currentDay
          setLightText:![self dateInCurrentMonth:self.currentDay.date]];
      ...
      if ([self.delegate
              respondsToSelector:@selector(datePicker:selectedDate:)]) {
    ...
    if (![_selectedDates containsObject:[self dateWithOutTime:dateDay.date]]) {
      ...
      if ([self.delegate
              respondsToSelector:@selector(datePicker:deselectedDate:)]) {
  ...
  if (dateInDifferentMonth) {
    [self slideTransitionViewInDirection:
              [dateDay.date timeIntervalSinceDate:firstOfCurrentMonth] < 0
                  ? UISwipeGestureRecognizerDirectionRight
                  : UISwipeGestureRecognizerDirectionLeft];
  }
}
...
- (void)slideTransitionViewInDirection:(UISwipeGestureRecognizerDirection)dir {
  ...
  UIView *newView = self.calendarDaysView =
      [[UIView alloc] initWithFrame:inStartFrame];
...
/* If _disableYearSwitch is true, then only the month switches are active, and
 * they'll swipe left and right only */

- (IBAction)nextMonthPressed:(id)sender {
  [self incrementMonth:1];
  [self slideTransitionViewInDirection:
            (_disableYearSwitch ? UISwipeGestureRecognizerDirectionLeft
                                : UISwipeGestureRecognizerDirectionUp)];
}
...
- (IBAction)prevMonthPressed:(id)sender {
  [self incrementMonth:-1];
  [self slideTransitionViewInDirection:
            (_disableYearSwitch ? UISwipeGestureRecognizerDirectionRight
                                : UISwipeGestureRecognizerDirectionDown)];
}
      ...
      if (_autoCloseOnSelectDate) {
        ...
        dispatch_after(
            dispatch_time(DISPATCH_TIME_NOW,
                          (int64_t)(self.autoCloseCancelDelay * NSEC_PER_SEC)),
...
- (void)showClearButton {
  ...
  if (_clearAsToday) {
    ...
    [self.clearBtn setTitle:NSLocalizedString(
                                @"TODAY", @"Customize this for your language")
                   forState:UIControlStateNormal];
  } else {
    [self.clearBtn
        setImage:(SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"8.0")
                      ? [UIImage imageNamed:@"dialog_clear"
                                                 inBundle:[NSBundle
                                                              bundleForClass:
                                                                  self.class]
                            compatibleWithTraitCollection:nil]
...
- (BOOL)dateInFutureAndShouldBeDisabled:(NSDate *)dateToCompare {
  NSDate *currentDate =
      [(self.isHistoryFutureBasedOnInternal ? self.internalDate
                                            : [NSDate date])dateWithOutTime];
  ...
  currentDate = [calendar
      dateFromComponents:[calendar components:comps fromDate:currentDate]];
  dateToCompare = [calendar
      dateFromComponents:[calendar components:comps fromDate:dateToCompare]];
  NSComparisonResult compResult = [currentDate compare:dateToCompare];
  return (compResult == NSOrderedDescending && _daysInHistory &&
          _daysInHistory <= dayDifference) ||
         (compResult == NSOrderedAscending && _daysInFuture &&
          _daysInFuture <= dayDifference);
}
...
- (BOOL)dateInCurrentMonth:(NSDate *)date {
  ...
  NSDateComponents *comp1 =
      [_calendar components:unitFlags fromDate:self.firstOfCurrentMonth];
